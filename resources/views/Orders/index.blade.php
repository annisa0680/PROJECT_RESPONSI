            ...
            <div class="cart-items">
                ...
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <div class="d-flex align-items-center">
                        <img src="{{ $item->product->image }}" alt="{{ $item->product->name }}" class="img-thumbnail me-3" style="width: 80px;">
                        <div>
                            <h6 class="mb-0">{{ $item->product->name }}</h6>
                            <small>Size: {{ $item->product->size }}</small><br>
                            <small>Quantity: {{ $item->quantity }}</small><br>
                            <small>by {{ $item->product->vendor }}</small>
                        </div>
                    </div>
                    <div>
                        <p class="mb-0">${{ number_format($item->product->price * $item->quantity, 2) }}</p>
                        <a href="{{ route('cart.destroy', $item->id) }}" class="text-danger">Remove</a>
                    </div>
                </div>
                <hr>
                @endforeach
